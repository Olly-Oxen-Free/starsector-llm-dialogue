...
public class TransferFuelAction implements StarlogueAction {
    private static final Logger log = Logger.getLogger(TransferFuelAction.class);
    private static final float MAX_TRANSFER = 4000f;

    @Override
    public String getId() {
        return "transfer_fuel";
    }

    @Override
    public String getDescription() {
        return "Transfer fuel between fleets after terms are agreed. "
    ...
    public java.util.Map<String, String> getParameterDescriptions() {
        ...
        d.put("quantity", "Units of fuel to transfer. Positive number.");
    ...
    public void execute(GameContext ctx, Map<String, Object> args) {
        ...
        if (npcToPlayer) {
            float actual = Math.min(amount, npc.getCommodityQuantity(Commodities.FUEL));
            if (actual <= 0f) return;
            npc.removeCommodity(Commodities.FUEL, actual);
            player.addCommodity(Commodities.FUEL, actual);
            log.debug("Starlogue: transfer_fuel npc_to_player=" + (int) actual);
        } else {
            float actual = Math.min(amount, player.getCommodityQuantity(Commodities.FUEL));
            if (actual <= 0f) return;
            player.removeCommodity(Commodities.FUEL, actual);
            npc.addCommodity(Commodities.FUEL, actual);
            log.debug("Starlogue: transfer_fuel player_to_npc=" + (int) actual);
        }
    }
